...
<task type="auto">
  ...
  <action>
    ...
    3. Update `client/src/App.tsx`:
       - Import ReportsPage and ReportDetailPage (add ReportDetailPage import even though page is created in Task 2 -- just add the import and route)
       - Add routes inside the AppLayout/ProtectedRoute block:
         - `<Route path="/reports" element={<ReportsPage />} />`
         - `<Route path="/reports/:reportId" element={<ReportDetailPage />} />`
       - Place these before the `<Route path="/" ...>` catch-all redirect
...
<task type="auto">
  ...
  <action>
    Create `client/src/pages/ReportDetailPage.tsx`:
