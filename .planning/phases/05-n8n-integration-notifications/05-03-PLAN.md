...
<task type="auto">
  <name>Task 2: Start outbox processor on server boot and add internal notification creation endpoint</name>
  <files>
    server/src/server.ts
    server/src/modules/notification/notification.service.ts
  </files>
  <action>
    ...
    **notification.service.ts** — Add a `createNotificationFromInternal` function:
    - This is a variant of `createNotification` that accepts the same input but is designed to be called from the internal API controller (n8n callbacks). It differs only in that it does NOT need fire-and-forget wrapping (the internal API controller handles errors via try/catch + next).
    - Signature: `createNotificationFromInternal(data: CreateNotificationInput): Promise<INotificationDocument>`. Throws on failure instead of returning null.
    - Export `CreateNotificationInput` type for reuse.
    - This enables a future internal endpoint `POST /api/v1/internal/notifications` that n8n can call to create in-app notifications after sending emails.
  </action>
  <verify>
    Run `cd server && npx tsc --noEmit` -- TypeScript compiles. Verify server.ts imports and calls `startOutboxProcessor`. Verify server.ts shutdown calls `stopOutboxProcessor`. Verify notification.service.ts exports `createNotificationFromInternal`.
  </verify>
  <done>
    Outbox processor starts automatically on server boot (10-second polling interval) and stops cleanly during graceful shutdown. Internal notification creation function is available for n8n callbacks. Events accumulate safely in dev without n8n running.
  </done>
</task>
...
6. notification.service.ts exports createNotificationFromInternal for n8n callbacks
</verification>
